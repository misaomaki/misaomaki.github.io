...
(function(window) {
    ...
    function SoundManager(smURL, smID) {
      ...
      // added by nixor  09/28/2012
      this._fadeTimer = null;
      this.fadeIn = function (amount) {
          _t.setVolume(0, true);
          //_t.play();
          _t.resume();
          this._fadeIn(amount);
      }
      this._fadeIn = function (amount) {
          var vol = _t._iO.volume;
          if (vol >= Math.min(_t.options.volume, 100)) return false;
          _t.setVolume(Math.min(vol + amount, 100), true);
          clearTimeout(_t._fadeTimer);
          _t._fadeTimer = setTimeout(function () {
              _t._fadeIn(amount);
          }, 20);
      };
    
      this.fadeOut = function (amount) {
          var vol = _t._iO.volume;
          if (vol == 0) {
              //this.stop();
              _t.pause();
          ...
          _t.setVolume(Math.max(vol - amount, 0), true);
          clearTimeout(_t._fadeTimer);
          _t._fadeTimer = setTimeout(function () {
              _t.fadeOut(amount);
          }, 20);
